...
This is the material that used to live in `README.md` before it was
narrowed down to intro, building, the two-configuration benchmark, and
supported model formats. Nothing here was dropped in that move — it is
the full record: every engine and card comparison, the third model, the
scope and non-goals, how the served artifacts are produced and calibrated,
the feature list, the milestone ledger, deployment notes, and why arcint
exists instead of an established engine.
...
quantisation and different card, so read it as a rough bound rather than a
row of the series above:
...
| 0.4.1 GGUF at the card's speed | tagged — the mixed form (Q4_K repacked with exact mins, Q5_K/Q6_K native, 16.54 GiB) 907 / 18.4 t/s prefill / decode at 856 tokens and 451 / 11.3 at 71.7k on the 24 GB card against the IR's 1,609 / 23.1 and 552 / 16.5 (`+p11`, DESIGN §7.0.2ba–bn); the gate met at depth, missed at 1k prefill (1.77× against 1.5×) and at the mixed form's 1k decode (1.27×; `--gguf-mode native` meets it at 15.22 GiB); 0.4.2–0.4.4 carry the rest: the repacked set's gemm, the tiled kernel's next 30 %, the Q5_K decode rate |
| 0.4.2 the repacked set's gemm | tagged — the runtime's int4 gemm measured at 88 % of the card's f16 matrix roof (its disassembly beside the IR's int8 kernel), int8 activations dead at the K-quant's 32-wide groups (DESIGN §7.0.2bo): the 1k prefill gate is not reachable through this gemm. `--gguf-mins split` (the min term as a separate term, exact-class): decode 53.5 / 72.2 ms at 856 / 71.7k tokens (from 55.05 / 73.3), 16.30 GiB, warm prefill 967 t/s (exact 948), 10/10, the 1k output byte-identical; every 0.4.1 1k prefill figure was a first request (907 = 948 warm) |
| 0.4.3 the tiled kernel's activation reads | tagged — plugin patch 0030 (`+p12`): the tiled K-quant kernel reads its activation block 32 rows per 2D message on Xe2 (128 → 32 messages per super-block per subgroup), 16 subgroups per work-group there, Q5_K on a 128-row tile (DESIGN §7.0.2bp): the timing test's gate 3.37 → 2.86 ms, Q6_K down 4.40 → 3.59 at 856 rows; served, the exact mixed form's warm 856-token prefill 940 → 1,001 t/s and 71.7k 451 → 464 (the first form over the depth bar), decode steps unchanged, byte-identical, 10/10; the handoff's 30 % timing gate missed at 15–18 %, measured and recorded |
...
recorded above), **0.4.1 took the GGUF path to 68 % / 98 % of the operator's prefill bars and, in its native form, to the decode bar** (`docs/milestone-0.4.1.md`; 0.4.2 measured the repacked set's gemm at the card's f16 rate and moved the min term out of it as a decode lever, DESIGN §7.0.2bo; 0.4.3 took the tiled kernel's activation reads to 32 rows for +6.5 % of warm 1k prefill, DESIGN §7.0.2bp; 0.4.4 the Q5_K decode rate) and **0.5.0 serves Qwen Flash Next**
