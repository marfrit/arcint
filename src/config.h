...
struct Config {
    ...
    int         gguf_mins = 0;        // --gguf-mins exact|shared|nibble|split: the repacked projections' mins packing (0 exact, +12.5 % on the Q4_K set; 1 shared, +6.25 %, inexact; 2 nibble, +3.1 %, inexact; 3 split, the min term a separate small MatMul on the group sums instead of augmented columns, exact-class, a decode lever; DESIGN 7.0.2bl, 7.0.2bo)
