...
Runs export_flash_next.py's transformers-support check (not the full
export -- no need to touch the network or a GPU to know whether qwen4_exp
has landed yet) and appends one line to a log, so the refusal is a
standing, dated record instead of something re-discovered by hand every
time someone asks "has upstream shipped it yet". The log never says who
ran it or from where -- see docs/design-qwen-flash-next.md, FIX A.
...
from export_flash_next import check_transformers_support  # noqa: E402
...
def run_once(log_path):
    ...
    try:
        ...
        line = f"{stamp} OPEN->CLOSED transformers {version} now carries qwen4_exp -- export path exists, re-run export_flash_next.py for real"
    except AssertionError as exc:
        line = f"{stamp} still blocked: {exc}"
