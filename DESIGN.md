...
| Acceptance | once per release, before the tag | hours | <!-- BEGIN GENERATED by tools/acceptance_manifest.py; verify with its check mode -->`coder-offload-1lane` gates byte-equality: cold vs warm cache, chunked vs unchunked prefill, one chunk size vs another (reports chunk sweep); `coder-offload-2lane` gates the same byte-equality claims as coder-offload-1lane, held at --parallel 2; `coder-offload-concurrency` gates no cross-slot bleed, cold/warm per lane, cancellation, admission (§4.1); `coder-served-large` gates steady-state decode >= 60 t/s (measured 53.4 cold / 69.2 warm, DESIGN §7.0.2ai) (reports 53.4 t/s cold, 69.2 t/s warm); `coder-served-small` gates the equivalence and concurrency suites both pass on the 16 GiB card (reports 48.0/49.5 t/s); `agent-dense` gates MTP identity (equivalence's MTP section) and MTP acceptance > 10%; `tier-reference-cell` gates tier ON/OFF byte-identity and E2 (CONT after history == CONT fresh) (reports 16.4 t/s decode, 26.6 t/s prefill) [runner lands in Increment 2]; `ngram-determinism-repeat` gates six fresh processes produce identical output [runner lands in Increment 2]; `depth-ladder` gates the load completes at both KV precisions on both cards (reports t/s per card and precision) [runner lands in Increment 2]; `pruefstand` gates 10/10 on the production coder artifact through the deployed package; `sanitizers` gates zero sanitizer reports [runner lands in Increment 2]; `package-build` gates the package build succeeds, is version-stamped, and its RPATH probe passes; a post-deploy smoke run follows the install, outside this cell <!-- END GENERATED by tools/acceptance_manifest.py --> |
