...
`the dev host's /models/ov/` on 2026-08-28.
...
- **Host memory is part of the measurement, not a footnote.** Deep-context runs
  ...
  a watchdog on it. Nothing about this is visible from inside the dev host.
- **Sanitizers**: clean under ASan + UBSan with `-fno-sanitize-recover`, unit
  ...
  the dev host: it aborts at startup on the aarch64 build container (allocator
  ...
  a clean ASan report on the dev host and disappears with the fix.
