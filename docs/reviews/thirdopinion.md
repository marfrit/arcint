...
Reviewed: `the dev workstation:~/src/arcint` at HEAD `f96d49e` **plus the dirty
...
**Not verifiable from here:** the measurement claims (they were taken on the dev host;
the review agent has neither the cards nor cmake). The suite's *structure* — red-before-green
...
*Generated 2026-08-28 by the review agent. Repo clones: ~/ref/{ninfer,ov,openvino-genai,vllm,lcpp} on the review agent (moved from /tmp/ref on 2026-08-28); per-repo technique extractions in ~/ref/reports/. Hardware
